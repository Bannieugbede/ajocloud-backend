...
## 2026-09-04 — Wiring the domain events that emit notifications

- **The inbox was empty in practice.** Push, in-app and email delivery all worked, preferences gated all four channels, and short-form copy existed for twelve product templates — but `grep '\.notify('` across `src/` returned nothing outside tests. The whole path was built and nothing entered it.
- Three events emit now, chosen because they are the ones with both a working code path and a template: an Ajo payout sent, a wallet funded by a settled deposit, and a KYC decision. Contribution reminders and Akawo progress need a scheduler rather than a request, so they stay out; Food distribution and Bill Payment have templates but no emitter yet.
- **Every send happens after its transaction commits, never inside it.** A notification sent from within announces money that can still roll back, and it cannot be unsent. In the payout path this meant restructuring the service to return the notification facts from the transaction and act on them in the wrapper that already existed for the HELD write.
- **None of them is awaited.** The money has moved by that point, so a push provider being down must not make a completed payout or a settled deposit look failed to its caller — for the webhook path in particular, that would be logged as a settlement failure for a deposit that had in fact settled. The tests prove this rather than assume it: an early version of the "still settles when notifying fails" test passed even with the call awaited, so it was replaced with one holding an unresolved promise and asserting the response arrives first. Mutation confirmed the new one fails when the call is awaited.
- The wallet-funded copy quotes the credited amount, not the gross. A ₦15,000 deposit that nets ₦14,675 must not announce ₦15,000 when that is not what the balance will show. Mutation confirmed that test catches the gross.
- The KYC notification carries none of the reviewer's reason. It is written for an internal audit trail, and a push payload crosses Apple's and Google's infrastructure and appears on a lock screen. Only APPROVE and REJECT notify: REQUEST_INFORMATION and ESCALATE move a profile without concluding it, and "verification needs attention" for an internal escalation would be both alarming and untrue.
- Dedupe keys are derived from the thing that happened, not from the request: the payout id, the payment intent id, and the KYC decision instant. A redelivered webhook or a retried execution therefore cannot notify twice, while a profile genuinely re-decided later does notify again.
- `formatMoney` is new because notification copy is the first place the backend renders an amount for a person rather than another system. Integer arithmetic on the minor units throughout — a float divide by 100 loses precision inside the range a group pool can reach, which its test pins at 2^53 kobo. `formatDueDate` renders in the group's own timezone, so a contribution due on the 1st in Lagos is not announced as due on the 31st.
- Files: `src/modules/notifications/domain/notification-money.ts`, `src/modules/ajo-groups/ajo-settlement.service.ts`, `src/modules/payments/payment-settlement.service.ts`, `src/modules/kyc/kyc-review.service.ts`, plus the three module wirings.
