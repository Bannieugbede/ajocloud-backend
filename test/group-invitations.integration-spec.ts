...
describeWithDatabase('Group invitations (PostgreSQL integration)', () => {
  ...
  beforeAll(async () => {
    ...
    [ownerId, joinerId, secondJoinerId, strangerId, listedJoinerId] = await Promise.all([
      user('owner'),
      user('joiner'),
      user('joiner2'),
      user('stranger'),
  ...
  it('lets anyone join a listed group by its public code, and nobody once unlisted', async () => {
    const group = await prisma.ajoGroup.findUniqueOrThrow({ where: { id: groupId } });
    // Generated by the database, not the application.
    expect(group.shortCode).toMatch(/^[2345679ACDEFGHJKMNPQRTUVWXYZ]{7}$/);

    // Unlisted, the public code is just a wrong code.
    await expect(invitations.preview(group.shortCode)).rejects.toThrow(
      'This invitation is no longer valid',
    );

    await groups.setListing(ownerId, groupId, true);
    await expect(invitations.preview(group.shortCode.toLowerCase())).resolves.toMatchObject({
      kind: 'listed',
      shortCode: group.shortCode,
    });
    await expect(invitations.resolveGroup(group.shortCode)).resolves.toMatchObject({ groupId });
    await expect(
      groups.join(listedJoinerId, groupId, { invitationCode: group.shortCode, requestedSlots: 1 }),
    ).resolves.toMatchObject({ slots: 1 });

    await groups.setListing(ownerId, groupId, false);
    await expect(invitations.preview(group.shortCode)).rejects.toThrow(
      'This invitation is no longer valid',
    );
  });
